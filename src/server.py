...
def _remove_online_by_socket(sock):
    with clients_lock:
        for uname, (s, a, h, old_a, old_h) in list(online_clients.items()):
            if s is sock:
                try:
    ...
    try:
        while True:
            ...
            while b'\n' in buffer:
                ...
                if not decrypted_json_str and old_session_aes_key:
                    decrypted_json_str = _decrypt(payload_bytes, old_session_aes_key, old_session_hmac_key)
                ...
                if session_start_time and session_aes_key and action != "rekey_start":
                    is_expired_time = (datetime.now() - session_start_time).total_seconds() > 3600
                ...
                try:
                    if action == "handshake":
                        ...
                        new_session_aes_key = derived_keys[:32]
                        new_session_hmac_key = derived_keys[32:]
                        
                        server_public_key_pem = server_public_key.public_bytes(Encoding.PEM, PublicFormat.SubjectPublicKeyInfo)
                        response = {"action": "rekey_finish", "payload": {"server_dhe_public_key": server_public_key_pem.decode('utf-8')}}
                        
                        current_aes_key_to_use = old_session_aes_key if old_session_aes_key else session_aes_key
                        current_hmac_key_to_use = old_session_hmac_key if old_session_hmac_key else session_hmac_key

                        encrypted_response = _encrypt(json.dumps(response), current_aes_key_to_use, current_hmac_key_to_use)
                        client_socket.sendall(encrypted_response + b'\n')
                        
                        session_aes_key = new_session_aes_key
                        session_hmac_key = new_session_hmac_key
